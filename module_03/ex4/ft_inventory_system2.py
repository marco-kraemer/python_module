...
class Inventory:
    ...
    def get_player_inventory(self, player_inventory):
        ...
        for item in player_inventory:
            category = inventory.items[item]["category"]
            rarity = inventory.items[item]["rarity"]
            quantity = player_inventory[item]
            price = inventory.items[item]["price"]
            str += f"{item} ({category}, {rarity}): {quantity}x @ {price} gold each = {price * quantity} gold\n"
        return str
    ...
    def get_player_inventory_value(self, player_inventory):
        ...
        total_items = 0
        for item in player_inventory:
            total_value += inventory.items[item]["price"] * player_inventory[item]
